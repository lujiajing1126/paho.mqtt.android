...
class MqttConnection implements MqttCallbackExtended {
    ...
    private static final RuntimeException PingSenderInitError = new RuntimeException("Create Ping Sender Failed");
    ...
    public void connect(MqttConnectOptions options, String invocationContext,
        ...
        try {
            ...
            // if myClient is null, then create a new connection
                ...
                pingSender = MqttConnection.this.initializePingSender(service);
                if (pingSender == null) {
                    throw PingSenderInitError;
                }
    ...
    private MqttPingSender initializePingSender(MqttService mService) {
        try {
            Constructor c = service.getPingSender().getDeclaredConstructor(MqttService.class);
            return (MqttPingSender) c.newInstance(mService);
        } catch (Exception ex) {
            Log.e(TAG, "initializePingSender: ", ex);
        }
        return null;
    }
}
